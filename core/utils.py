...
def get_company_report_data(company):
...
def export_company_pdf(company):
...
def export_company_excel(company):
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        ...
        if company.note:
            worksheet.write(0, 0, f"Забележка: {company.note}", workbook.add_format({'bold': True, 'font_color': 'red'}))
            start_row = 3
        else:
            start_row = 0
        ...
        for r_idx, row_data in df_for_excel.iterrows():
            ...
            worksheet.write(r, 12, row_data['Километри'] if row_data['Километри'] is not None else "", text_format)
            worksheet.write(r, 13, str(row_data['Billing Document']), text_format)

        # Write summary
        summary_start_row = len(df_for_excel) + start_row + 2
        worksheet.write(summary_start_row, 0, f"Общи литри по продукт / {company.name}", workbook.add_format({'bold': True, 'font_size': 14}))
        ...
        total_format_amount = workbook.add_format({'bold': True, 'bg_color': '#FFFF66', 'border': 1, 'num_format': '#,##0.00'})
        total_format_qty = workbook.add_format({'bold': True, 'bg_color': '#FFFF66', 'border': 1, 'num_format': '#,##0.00'})
...
def export_single_company_zip(company):
    zip_buffer = io.BytesIO()
    ...
    with db_transaction.atomic():
        for _, row in df.iterrows():
            ...
            Transaction.objects.create(
                plant=str(row.get('Plant', "")).strip() if pd.notna(row.get('Plant')) and str(row.get('Plant')).lower() != 'nan' else "",
                card_number=card_number,
                card=card,
                material=material,
                date=date_parsed.date(),
                bill_qty=qty,
                bill_qty2=str(row.get('Bill.qty.1', "")).strip() if pd.notna(row.get('Bill.qty.1')) and str(row.get('Bill.qty.1')).lower() != 'nan' else "",
                price=price,
                amount=amount,
                auth_time=auth_time,
                km_stand=km_stand,
                billing_document=str(row.get('Billing Document', "")).strip() if pd.notna(row.get('Billing Document')) and str(row.get('Billing Document')).lower() != 'nan' else ""
            )
